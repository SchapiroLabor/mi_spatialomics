...
samples %>% walk(\(sample){
  ...
  run_misty(misty.views.cts, sample, paste("results_cts_",size_param,".sqm",sep=""), bypass.intra = TRUE)
})
```
...
misty.results.g <- groups %>% map(~ collect_results("results_cts_125.sqm", .x))
...
dir.create(paste("misty_figures_",size_param,sep="")) ##
...
misty.results.g %>% iwalk(\(misty.results, cond){
  ...
  ggsave(paste0("misty_figures_125/", cond, "_stats.pdf"), width = 10, height = 10)
...
names(misty.results.g) %>% walk(\(g){
  ...
  # write_graph(out, paste0("graphs/para.125.", g, ".graphml"), "graphml")
  #write_graph(out, paste0("graphs/para.",size_param,".", g, ".graphml"), "graphml")
})
```

```{r}
## Check directionality of interaction
misty.results.g$"2d" %>% plot_interaction_heatmap(view = "paraview", cutoff = 0.4)
```
