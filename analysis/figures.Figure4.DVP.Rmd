...
manual_labeled_proteins <- c("Thbd","Reln","Tns","Vwf","Fga","Ltf","S100a9","Lcp1")
...
volc_limma_IZ_remote <- plot_pretty_volcano(limma_mi_remote, 
  ...
  geom_label_repel(box.padding = 0.75, max.overlaps = Inf, size = 3)
...
endo_proteomic_corr <- ggplot(snrna_prot,aes(avg_log2FC,logFC,
  ...
  geom_point(data = subset(snrna_prot,gene == "Vwf"),size = 4, fill = "purple", pch = 21) +
  geom_point(data = subset(snrna_prot,gene %in% c("Npr3","Cdh11")),size = 4, fill = "darkorange", pch = 21) +
...
joined_plot <- npr3_plot + vwf_plot
...
vwf_vlnpot <- SCpubr::do_ViolinPlot(sample = sub_human_cite, 
 ...
 vwf_vlnpot <- vwf_vlnpot + theme(plot.margin=unit(c(0,0,0,0), "pt"))
