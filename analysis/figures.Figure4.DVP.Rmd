...
pca_plot <- ggplot(pcs,aes(PC1,PC2)) +
  ...
  ggforce::geom_mark_ellipse(color = "white",aes(fill = group, label = group)) +
...
endo_proteomic_corr <- ggplot(snrna_prot,aes(avg_log2FC,logFC,
  ...
  geom_point(data = subset(snrna_prot,gene %in% c("Npr3","Cdh11")),size = 4, fill = "lightblue", pch = 21) +
  geom_label_repel() +
...
save_plot(vwf_vlnpot,
          file = "./plots/Figure4.human_citeseq_vlnplot.png",
          base_height = 4)
```
