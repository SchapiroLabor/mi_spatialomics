...
def plot_pixie_maps(pixel_image, sample, mask_color, scale, screenshot_path,  crop_out_path):
    viewer = napari.Viewer()
    viewer.add_labels(pixel_image, color=mask_color)
...
def plot_layers_napari(image=None, image_channel_axis=0, image_channel_colors=None, points_data=None, genes_of_interest=None, roi=None, mask=None, color_palette=("green", "blue"), pt_size=1, output_path=".",
    ...
    if image is not None:
        if image.any():
            ...
            viewer.add_image(
                image_view, channel_axis=image_channel_axis, colormap=image_channel_colors)
    if mask is not None:
        if mask.any():
