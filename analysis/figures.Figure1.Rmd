...
```{r}
#### Source data
## Export UMAP mappings for Extended data source
umap_embeddings <- as.data.frame(seurat_object@reductions$umap@cell.embeddings)
cell_type_annotation <- seurat_object$anno_cell_type_lvl2
umap_embeddings$cell_types <- cell_type_annotation
umap_embeddings$cellID <- rownames(umap_embeddings)

write.csv(umap_embeddings, file = "./source_data/Figure1C.umap_embeddings.csv", row.names = FALSE)
```
...
```{r}
#### Source data
## Export spatial coordinates for Extended data source
spatial_coordinates <- meta_sample 
spatial_coordinates$cellID <- rownames(spatial_coordinates)
spatial_coordinates <- spatial_coordinates %>%
  
  select(cellID, X_centroid, Y_centroid, anno_cell_type_lvl2)

write.csv(spatial_coordinates, file = "./source_data/Figure1F.spatial_coordinates.csv", row.names = FALSE)
```
